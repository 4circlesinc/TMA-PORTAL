    ...
    render: function (s) {
      ...
      function plan(name, tagline, annualPrice, badges, current, forChip) {
        return '<article class="tma-portal-plan' + (current ? ' is-current' : '') + '">' +
          ...
          '<span><img src="/TMA-PORTAL/images/icons/phosphor/Users.svg" alt="">3 Employee Accounts</span>' +
          '<span><img src="/TMA-PORTAL/images/icons/phosphor/Database.svg" alt="">1TB/license Storage</span>' +
          '</div>' +
    ...
    render: function (s) {
      return '<div class="tma-portal-two-col">' +
        ...
        ui().section('We’re sorry to see you go!',
          ...
          '<div class="tma-portal-feature"><img src="/TMA-PORTAL/images/icons/phosphor/EnvelopeSimple.svg" alt="">' +
          ...
          '<div class="tma-portal-feature"><img src="/TMA-PORTAL/images/icons/phosphor/DeviceMobile.svg" alt="">' +
          ...
          '<div class="tma-portal-feature"><img src="/TMA-PORTAL/images/icons/phosphor/Signature.svg" alt="">' +
        ...
        (s.serviceTeams.length
          ? ui().table(['Team', 'Members', 'Created', ''], s.serviceTeams.map(function (t) {
              return '<tr><td><strong>' + ui().esc(t.name) + '</strong></td>' +
                ...
                '<button type="button" class="tma-portal-icon-btn" data-team-delete="' + t.id + '" title="Delete team" aria-label="Delete team"><img src="/TMA-PORTAL/images/icons/phosphor/Trash.svg" alt=""></button>' +
                '</div></td></tr>';
            }).join('')) +
        ...
        (s.customFields.length
          ? ui().table(['Field', 'Type', ''], s.customFields.map(function (f) {
              return '<tr><td><strong>' + ui().esc(f.name) + '</strong></td>' +
                ...
                '<td><div class="tma-portal-row-actions">' +
                '<button type="button" class="tma-portal-icon-btn" data-field-delete="' + f.id + '" title="Delete field" aria-label="Delete field"><img src="/TMA-PORTAL/images/icons/phosphor/Trash.svg" alt=""></button>' +
                '</div></td></tr>';
            }).join(''))
      ...
      return ui().alphaFilter(self.alpha) +
        ...
        ui().table(['', 'Name', 'Email', 'Company'], list.map(function (p) {
          return '<tr>' +
            ...
            '<td><span class="tma-portal-avatar-cell"><img src="/TMA-PORTAL/images/avatars/AvatarByewind.png" alt=""><strong>' + ui().esc(p.lastName + ', ' + p.firstName) + '</strong></span></td>' +
    ...
    render: function (s) {
      ...
      return '<h3 class="tma-portal-section__title">Add Connectors</h3>' +
        ...
        s.connectors.map(function (c) {
          ...
          var src = logo.brand ? '/TMA-PORTAL/images/icons/brands/' + logo.brand + '.svg' : '/TMA-PORTAL/images/icons/phosphor/' + logo.icon + '.svg';
        ...
        (s.folderTemplates.length
          ? ui().table(['Template', 'Subfolders', ''], s.folderTemplates.map(function (t) {
              return '<tr><td><strong>' + ui().esc(t.name) + '</strong></td>' +
                ...
                '<td><div class="tma-portal-row-actions">' +
                '<button type="button" class="tma-portal-icon-btn" data-ftpl-delete="' + t.id + '" title="Delete template" aria-label="Delete template"><img src="/TMA-PORTAL/images/icons/phosphor/Trash.svg" alt=""></button>' +
                '</div></td></tr>';
            }).join(''))
        ...
        (s.remoteUploadForms.length
          ? ui().table(['Form', 'Destination folder', 'Created', ''], s.remoteUploadForms.map(function (f) {
              return '<tr><td><strong>' + ui().esc(f.name) + '</strong></td>' +
                ...
                '<td><div class="tma-portal-row-actions">' +
                '<button type="button" class="tma-portal-icon-btn" data-ruf-code="' + f.id + '" title="Get HTML code" aria-label="Get HTML code"><img src="/TMA-PORTAL/images/icons/phosphor/FileCode.svg" alt=""></button>' +
                '<button type="button" class="tma-portal-icon-btn" data-ruf-delete="' + f.id + '" title="Deactivate form" aria-label="Deactivate form"><img src="/TMA-PORTAL/images/icons/phosphor/Trash.svg" alt=""></button>' +
                '</div></td></tr>';
            }).join(''))
  ...
  function renderNav() {
    return NAV.map(function (n) {
      ...
      return '<button type="button" class="tma-portal-admin__nav-item" data-admin-group="' + n.group + '" aria-expanded="' + open + '">' + ui().esc(n.label) +
        '<img class="tma-portal-admin__caret" src="/TMA-PORTAL/images/icons/phosphor/CaretRight.svg" alt=""></button>' +
        (open
