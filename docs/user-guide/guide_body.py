...
def _activity(doc, add_title, add_h2, add_body, add_bullets, add_table, add_callout):
    ...
    add_body(
        doc,
        "Typical messages include Your sign-in code, password reset, password changed, two-factor changed, invitations, and CIP notices. Wording can be edited by administrators under Templates → System Emails. Support copy in those messages uses support@tmantoinelaw.com.",
    )
...
def _faq(doc, add_title, add_h2, add_body):
    ...
    add_body(
        doc,
        "Email support@tmantoinelaw.com. A phone number for the helpdesk is not published in the portal. [Information Required] if your firm uses a different internal contact.",
    )
