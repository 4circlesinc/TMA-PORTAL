...
| Auto-update | `updater.js` polls `/desktop/releases` (`{mac:{available,version,minOs,url},windows:{…},android:{…}}`, `DesktopReleasesController.php`), downloads, installs on relaunch | Same `/desktop/releases` feed: `web/AppUpdater.kt` polls hourly, downloads the APK, prompts "A new version is available" and hands it to the system installer (`REQUEST_INSTALL_PACKAGES`). First sideload from the website still has to be done once so the phone has a build that knows how to check. |
...
4. Verify from Laravel Cloud's Commands panel (no shell needed): `php artisan push:status` reports the project, whether Google accepts the service account, and the registered devices; `php artisan push:status --test=you@firm.com` sends a real test notification to that person's phones.
