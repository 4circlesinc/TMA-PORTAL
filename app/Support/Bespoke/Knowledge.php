...
final class Knowledge
{
    ...
    public static function pageSummary(array $identity, array $page): string
    {
        ...
        $copy = match ($kind) {
            ...
            'cip-list' => $cip
                ? 'CIP Applications. Open a number to work the file. Staff: Create New Application offers Pre-Approval, Post-Approval, Add-On, New service provider, and Import. Provider contacts file Pre-Approval, Post-Approval, and Add-On.'
    ...
    public static function statuses(): array
    {
        return [
            ...
            'Pending Review' => 'Waiting on review / compliance. Next: Non-compliant or Background Check (Add-On: Non-compliant, Approved, or Denied — no Background Check).',
            'Non-compliant' => 'Did not meet a requirement. Can return to Pending Review or move to Background Check (Add-On: return to Pending Review, or Approved / Denied).',
            'Background Check' => 'Checks in progress. From here: Non-compliant, Delayed, Approved, or Denied. Not used on Add-On.',
            'Delayed' => 'Held up. From here: Non-compliant, Approved, or Denied. Not used on Add-On.',
            'Approved' => 'Pre-approval grant. Record decision uses Granted letter templates. Next: Post-Approval, or New Appeal. An approved Add-On stays on its AO number and does not enter Post-Approval.',
    ...
    public static function catalogue(): array
    {
        return [
            ...
            [
                'id' => 'addon-submit',
                'q' => 'How does Add-On confirmation and submission work?',
                'keywords' => ['add-on submit', 'confirm submission', 'ready to submit', 'assessment feedback', 'pending review', 'add-on submission'],
                'cipOnly' => true,
                'answer' => "After every required Add-On document is judged, the file moves through **Assessment Feedback**. If any document needs work it becomes **Updates Required** (the authorized agent is notified and can still edit). If none do, it progresses toward submission.\n\nWhen every required document is **Ready for submission**, the application automatically becomes **Ready to Submit** and the agent is told to press **Confirm submission**. That locks the original package read-only and writes an audit record; Assessment Feedback and Additional Documents stay writable.\n\nStaff then **Record submission** with the CIP application number the Unit issued the Add-On itself (not the parent's) and the submission date. From then on every screen shows the Add-On's own CIP number; the AO reference stays for audit. The portal stores who recorded it and the Add-On type, moves the file to **Pending Review**, and keeps the locked package for audit.",
            ],
            [
                'id' => 'addon-query-decision',
                'q' => 'How do Add-On additional document requests and decisions work?',
                'keywords' => ['add-on query', 'additional documents', 'non-compliant', 'add-on decision', 'approved', 'denied', 'decision history'],
                'cipOnly' => true,
            ...
            [
                ...
                'cipOnly' => true,
                'adminOnly' => true,
                'answer' => "Only **administrators** assign CIP files to staff reviewers. Open the file in [CIP Applications](/citizenship-applications) → **Assign an officer**. A New file moves to **Review Applications** and the officer is emailed.\n\nOn an Add-On that email names the Add-On reference number, parent CIP number, main applicant, Add-On applicant, and a portal link.\n\nThat is not how you attach a service provider. Pick **Service provider** on the intake form when you create the application. To move a filed file to another firm later, an administrator uses **Change service provider** on the row menu — the folder moves with it and the outgoing firm loses access.",
            ],
            [
                'id' => 'invite-provider',
            ...
            [
                ...
                'answer' => 'Email support@tmantoinelaw.com. Include the page, account email, and application number if it is a CIP file. Do not send sign-in codes or authenticator codes.',
            ],
            [
                'id' => 'provider-assign',
                'q' => 'How do I assign a post-approval file to a service provider?',
                'keywords' => ['post-approval', 'assign provider', 'reassign', 'galaxy', 'transfer provider', 'change service provider'],
                'cipOnly' => true,
                'staffOnly' => true,
                'answer' => 'Pick the firm on Create New Application (including Create New Post-Approval Application). A file already Approved in this portal keeps that firm when you move it into Post-Approval. Administrators can later use **Change service provider** on the application row: confirm carefully — the application folder moves under the new firm and the current service provider loses all access to the files and the application. Assign an officer is only which staff member reviews the file.',
            ],
            [
                'id' => 'compose-updates',
