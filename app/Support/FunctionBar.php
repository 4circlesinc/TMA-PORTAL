...
class FunctionBar
{
    ...
    public static function preset(): array
    {
        if (self::$preset === null) {
            self::$preset = json_decode(
                file_get_contents(base_path('design/function-bar.json')),
                true
            );
        }
    ...
    public static function selectionLabel(string $variant, int $selectedCount): ?string
    {
        $variantTokens = self::variantTokens($variant);
        $template = $variantTokens['selectionLabel'] ?? null;

        if ($template === null) {
            return null;
        }

        return str_replace('{count}', (string) $selectedCount, $template);
    }
}
