...
(function () {
  ...
  function renderApplicationBar(state, app) {
    ...
    // Last: an appeal is what happens after everything above has finished.
    var appeal = renderAppealAction(state, app);
    if (appeal) parts.push(appeal);
    if (!parts.length) return '';
   ...
   *
   * Both are drawn from the same fields, so the two screens cannot disagree
  ...
  function renderClientFolderList(root, res) {
    ...
    files.forEach(function (f) {
      ...
      /*
       * A picture of the document, not a glyph for its type.
       *
       * These rows are mostly passports, photographs and scans, and a column
       * of identical green image marks tells the reader nothing about which
       * one they are looking at. The listing comes from the File Library API,
       * so it already carries the server's `thumbUrl`; TMAFileThumbs paints
       * page one for PDFs, which have no server thumbnail on this stack.
       */
  ...
  function cipOverrideFieldsHtml() {
    return '<div class="tma-dash__clients-field tma-dash__clients-field--stacked">' +
      ...
      '<label class="tma-dash__clients-field-label" for="cip-override-word">Type <span class="tma-dash__clients-override-word">' + CIP_OVERRIDE_WORD + '</span> to confirm</label>' +
  ...
  function openAppealRequestDialog(applicationId, clientUid) {
    ...
    ui.openModal({
      ...
      body:
        ...
        '<textarea id="cip-appeal-req" class="tma-dash__clients-field-textarea" data-cip-appeal-req' +
      ...
      onMount: function (el) {
        ...
        var save = el.querySelector('[data-cip-save-req]');
        if (!save) return;

        save.addEventListener('click', function () {
          var reasonEl = el.querySelector('[data-cip-appeal-req]');
  ...
  function wireEvents(root, state, scope, navigate, render) {
    ...
    MORPH.unwired(root, '[data-cip-appeal-start]').forEach(function (btn) {
      MORPH.on(btn, 'click', function () {
        var app = applicationFor(state.selectedId);
        if (!app) return;
        openAppealRequestDialog(app.id, app.clientUid);
      });
    });
